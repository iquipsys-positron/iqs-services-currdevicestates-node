...
export interface ICurrentDeviceStatesPersistence extends IGetter<CurrentDeviceStateV1, string>, IWriter<CurrentDeviceStateV1, string> {
    ...
    getOneById(correlationId: string, id: string, 
        callback: (err: any, item: CurrentDeviceStateV1) => void): void;

    set(correlationId: string, item: CurrentDeviceStateV1, 
        callback: (err: any, item: CurrentDeviceStateV1) => void): void;

    setBatch(correlationId: string, items: CurrentDeviceStateV1[],
